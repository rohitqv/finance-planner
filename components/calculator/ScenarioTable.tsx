      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {scenarios.map((s) => {
            const r = calculate(s);
            return (
              <tr key={s.id} className="border-t transition-colors first:border-t-0 hover:bg-gray-50">
                ...
                <td className="space-x-3 px-3 py-2 text-right">
                  <button className="text-gray-500 transition-colors hover:text-gray-800" onClick={() => onDuplicate(s.id)}>
                    Duplicate
                  </button>
                  <button className="text-red-500 transition-colors hover:text-red-700" onClick={() => onDelete(s.id)}>
                    Delete
                  </button>
                </td>
              </tr>
